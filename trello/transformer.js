...
{
	var buildDependencyOrientatedDataSet = function(cards,lists,checklists){
    ...
    var checklistsEnum = Enumerable.From(checklists[0]);
    ...
    checklistsEnum.Where(function(d){ return d.name.toLowerCase().indexOf('dependenc') === 0; }).ForEach(function(list){
      cardsEnum.Where(function(card){ return card.id === list.idCard }).ForEach(function(card, index){
        card.dependencyListId = list.id
        card.dependencyList = Enumerable.From(list.checkItems)
      });
    });
	...
	var getAllDependencies = function(nodes){
		return Enumerable.From(nodes)
						 .Select(function(d,i){ return getDependencies(d.desc,d.dependencyList);})
	...
	var getDependenciesForCard = function(dependencyId,nodes)
	{
		return Enumerable.From(nodes)
						 .Where(function(d){
								return Enumerable.From(getDependencies(d.desc,d.dependencyList))
	...
	var buildLinksFromDependencies = function(nodes)
	{
		...
		Enumerable.From(nodes).ForEach(function(d,i){
			var deps = getDependencies(d.desc,d.dependencyList);
	...
	var getDependencies = function(s,dependencyList){
    ...
    dependsPattern = /trello.com\/c\/([^\/]+)/g;
    if (dependencyList) dependencyList.ForEach(function(di) {
      while(match = dependsPattern.exec(di.name))
      {
        dependsOn.push(match[1]);
      }
    })
